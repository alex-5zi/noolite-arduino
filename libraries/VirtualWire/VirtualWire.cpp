...
void vw_pll()
{
	...
	if (vw_rx_active)
	{
	    if (++vw_rx_bit_count >= 8)
	    {
		uint8_t this_byte = (vw_rx_bits & 0xff);
		vw_rx_buf[vw_rx_len++] = this_byte;
		if (vw_rx_len >= VW_MAX_MESSAGE_LEN)
		{
	...
	// Not in a message, see if we have a start symbol 0xb38
	else if (vw_rx_bits == 0xaaaa)
	{
...
uint8_t vw_get_message(uint8_t* buf, uint8_t* len)
{
	...
	*len = rxlen;
    ...
    return true; //(vw_crc(vw_rx_buf, vw_rx_len) == 0xf0b8); // FCS OK?
}
